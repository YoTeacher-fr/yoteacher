...
class CurrencyManager {
     ...
     * The closed state is pixel-perfect identical to the original native select.
     */
    initSingleSelector(selector) {
        ...
        // Create trigger as <div> (not <button>) to avoid browser button defaults
        ...
        // Insert trigger before select, append panel to parent
        parent.insertBefore(trigger, selector);
        parent.appendChild(panel);

        // Hide native select accessibly (keep in a11y tree and forms)
    ...
    /**
     * Update native selects AND custom dropdown triggers
    ...
    _setupDropdownGlobals() {
        ...
        document.addEventListener('click', (e) => {
            if (!e.target.closest('.currency-dropdown-trigger') && !e.target.closest('.currency-dropdown-panel')) {
                this._closeAllDropdowns();
            }
        });
